...
*/
function IWmyrole_adminapi_delUserGroups()
{
 ...
 * @return:	True if success and false in other case
*/
function IWmyrole_adminapi_addUserToGroup($args)
{
	$dom = ZLanguage::getModuleDomain('IWmyrole');
	...
	if($defaultRoles == 1){
		...
		$defaultRoles = ModUtil::func('IWmain', 'user', 'userGetVar', array('uid' => $uid,
												'name' => 'defaultRoles',
												'module' => 'IWmyrole',
												'sv' => $sv));
 ...
 * @return:	True if success and false in other case
*/
function IWmyrole_adminapi_correctGroupPermissions($args)
{
	$dom = ZLanguage::getModuleDomain('IWmyrole');
	// Security check
	if (!SecurityUtil::checkPermission('IWmyrole::', "::", ACCESS_ADMIN)) {
...
function IWmyrole_adminapi_changePermissions($args){
